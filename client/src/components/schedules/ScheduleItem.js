...
const ScheduleItem = ({ schedule, deleteSchedule, fetchOccurrences, currencyCode }) => {
  ...
  const renderMenu = () => {
    return (
      <div ref={node} className="text-gray-900 absolute text-sm top-0 right-0 mt-6 bg-gray-100 shadow rounded">
        <ul>
          ...
          <li className="mb-1 p-2 pr-3 flex items-center cursor-pointer hover:bg-gray-200" onClick={onDeleteSchedule}>
            <svg
              fill="none"
              strokeLinecap="round"
              strokeLinejoin="round"
            ...
            <span className="ml-3 text-red-600">Delete</span>
          </li>
        </ul>
      </div>
    );
  ...
  return (
    <div className="w-full flex flex-col shadow mx-2" style={{ maxWidth: "16rem" }}>
      <div className="p-4 relative flex justify-between items-center rounded-t">
        <h4 className="text-sm">{schedule.description}</h4>
        <span className="cursor-pointer rounded-full p-1 hover:bg-gray-600" onClick={() => setMenuOpen(true)}>
          <svg
            fill="none"
            strokeLinecap="round"
            strokeLinejoin="round"
      ...
      <div className={`p-4 rounded-b ${styles[currentStyle].upperDiv}`}>
        <div className="mt-1 font-bold">
          <TransactionBadge amount={schedule.amount} currencyCode={currencyCode} />
        </div>
        <p className={`mt-1 text-xs ${styles[currentStyle].freqDesc}`}>{getFrequencyDescription(schedule)}</p>
      </div>
    </div>
  );
};
...
export default connect(mapStateToProps, { deleteSchedule, fetchOccurrences })(ScheduleItem);
